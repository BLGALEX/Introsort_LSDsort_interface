...
//Далее сортировка, переписанная для структуры с двумя полями типа int, сортировка проиходит по первому полю
...
/*void LSDSort_check(check_stable* first, int length)
{
	...
	for (int i = 1; i <= digits; i++)
	{
		...
		for (int j = 0; j < length; j++)
		{
			...
			buffer[indexes_of_numbers[x]] = first_u[j].key;
			indexes_of_numbers[x]++;
		}
		for (int j = 0; j < length; j++)
			first_u[j] = buffer[j];
	...
	for (int j = 0; j < length; j++)
		first[j].check = (first_u[j] + minimum_int);
}*/